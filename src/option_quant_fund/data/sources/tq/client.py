"""Non-connecting TqSdk client wrapper for TASK-007."""
...
class TqSdkNotInstalledError(ImportError):
    """The optional tqsdk package cannot be imported."""


def is_tqsdk_available() -> bool:
    """Return True if tqsdk is installed."""
    try:
...
class TqClient:
    """Adapter entry point. Live sessions are opt-in via connect()."""
    ...
    def connect(self) -> None:
        """Start a TqSdk session. Not used by default unit tests."""
        if self._connected:
            return

        if not is_tqsdk_available():
            raise TqSdkNotInstalledError(
                "tqsdk is not installed; install it before connect()"
            )
